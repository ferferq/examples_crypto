const express = require("express");
const bodyParser = require("body-parser");
const cors = require("cors");
const crypto = require("crypto");
const { importSPKI, generalVerify } = require('jose'); // Biblioteca jose para verificação de assinatura ECDSA

const app = express();
app.use(bodyParser.json());
app.use(cors());

const publicKeyPem = `-----BEGIN PUBLIC KEY-----

1wIDAQAB
-----END PUBLIC KEY-----`;
const privateKeyPem = `-----BEGIN PRIVATE KEY-----

u1bN4VzMWNDM+KBoT+qK
-----END PRIVATE KEY-----`;

app.get("/get-key", (req, res) => {
  res.send(publicKeyPem);
});

async function verifySignature(publicKeyBase64, data, signatureBase64) {
    try {
        const pemKey = `-----BEGIN PUBLIC KEY-----\n${publicKeyBase64}\n-----END PUBLIC KEY-----`;
        const key = await importSPKI(pemKey, "ES256");
        
        const encoder = new TextEncoder();
        const isValid = await generalVerify(encoder.encode(data), key, Buffer.from(signatureBase64, 'base64'))
            .then(() => true)
            .catch(() => false);
        
        console.log('isValid', isValid);
        return isValid;
    } catch (err) {
        console.log('error', err);
        return false;
    }
}

app.post("/decrypt", async (req, res) => {
    const { encryptedData, encryptedKey } = req.body;

    try {
        const privateKey = crypto.createPrivateKey(privateKeyPem);
        const encryptedBuffer = Buffer.from(encryptedKey, "base64");
        const decryptedKey = crypto.privateDecrypt(
            {
                key: privateKey,
                padding: crypto.constants.RSA_PKCS1_OAEP_PADDING,
                oaepHash: "sha256",
            },
            encryptedBuffer
        );
        const { key, iv, authTag } = JSON.parse(decryptedKey);

        const decipher = crypto.createDecipheriv(
            "aes-256-gcm",
            Buffer.from(key, "base64"),
            Buffer.from(iv, "base64")
        );
        decipher.setAuthTag(Buffer.from(authTag, "base64"));

        const encryptedBufferData = Buffer.from(encryptedData, "base64");
        const decryptedDataBuffer = Buffer.concat([
            decipher.update(encryptedBufferData),
            decipher.final(),
        ]);

        const decryptData = JSON.parse(decryptedDataBuffer.toString());

        const { data, signature, ecdsaPublicKey } = decryptData;
        console.log('fer', decryptData);
        const isValid = await verifySignature(ecdsaPublicKey, data, signature);
        if (isValid) {
            const { cardData } = JSON.parse(data);
            return res.json({ decryptedData: cardData, decryptedKey: { key, iv, authTag } });
        }

        return res.status(500).json({ error: "Invalid data", details: "Data was modified" });
    } catch (error) {
        return res.status(500).json({ error: "Failed to decrypt data", details: error.message });
    }
});

const PORT = 3000;
app.listen(PORT, () => {
    console.log(`Server on http://localhost:${PORT}`);
});